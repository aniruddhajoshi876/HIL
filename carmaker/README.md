...
| Path | What |
|---|---|
| `FS_race/src_cm4sl/` | Canonical CM4SL sources: `IO.c`, `User.c`, `User.h`, `security_cookie_stub.c`, `Makefile`, `TorqueVect.mdl`, `cmenv.m`. |
| `config/MFE26_Inverter_CarMaker.dbc` | The two-bus CAN contract (`0x500`-`0x506`, sensor and inverter frames). |
| `deploy/apply_cm4sl.ps1` | Copies the canonical CM4SL sources into a CarMaker project before an R2022a build. Safe: shows a plan, refuses to clobber unpreserved changes, never deletes. `-Verify` checks a deployed project. |
