...
1. **Confirmed — identify physical I/O first.** In the IO Configurator, identify a supported CarMaker-side CAN adapter/channel and activate it at 1 Mbaud. The Speedgoat IO614 is not CarMaker I/O. If this channel is otherwise unused, extend `FS_race/src/IO.c` in IPG-MFE to initialize the slot and CAN/CANFD module in `IO_Init()`, then rebuild the CarMaker executable. UsersGuide_HIL.pdf, section 4.5.2 “CAN”, p. 68.
