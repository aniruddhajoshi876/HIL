...
proc HandleDataVec {sid msgid} {
    ...
    ##.lights itemconfigure TrapId   -text [format " Loop %i: %.3f s" $Qu(TriggerPointId) $Qu(TriggerPointTime)]
    ##.lights itemconfigure LapTime  -text [format " %.3f s"       $Qu(LapTime)]
    ##.lights itemconfigure LapNo    -text [format " Lap Time: %i" $Qu(LapNo)]


    if {$Qu(TriggerPointId) == 1} {
        .lights itemconfigure Loop1   -text [format " Loop 1: %.3f s" 0.0]
        .lights itemconfigure Loop2   -text [format " Loop 2: %.3f s" 0.0]
        .lights itemconfigure Loop3   -text [format " Loop 3: %.3f s" 0.0]
        .lights itemconfigure Loop4   -text [format " Loop 4: %.3f s" 0.0]
        .lights itemconfigure Runup -text [format " Runup: %.3f s"  $Qu(TriggerPointTime)]
        set lastTriggerTime $Qu(TriggerPointTime)
    }
    if {$Qu(TriggerPointId) == 2} {
        ##if {![info exists lastTriggerTime]} {
        ##set lastTriggerTime 0
        ##}
        ##set newTriggerTime [expr {$Qu(TriggerPointTime) - $lastTriggerTime}]
        .lights itemconfigure Loop1   -text [format " Loop 1: %.3f s" $Qu(TriggerPointTime)]
        
    }
    if {$Qu(TriggerPointId) == 3} {
        .lights itemconfigure Loop2   -text [format " Loop 2: %.3f s" $Qu(TriggerPointTime)]
    }
    if {$Qu(TriggerPointId) == 4} {
        .lights itemconfigure Loop3   -text [format " Loop 3: %.3f s" $Qu(TriggerPointTime)]
    }
    if {$Qu(TriggerPointId) == 5} {
        .lights itemconfigure Loop4   -text [format " Loop 4: %.3f s" $Qu(TriggerPointTime)]
    }



    ##.lights itemconfigure Loop3   -text [format " Loop %i: %.3f s" $Qu(TriggerPointId) $Qu(TriggerPointTime)]
    ##.lights itemconfigure Loop4   -text [format " Loop %i: %.3f s" $Qu(TriggerPointId) $Qu(TriggerPointTime)]
    ##.lights itemconfigure Runup -text [format " %i: %.3f s"  $Qu(TriggerPointId) $Qu(TriggerPointTime)]
...
proc CreateDialog {} {
    ...
    ## Powermeter
    Tacho::Create .pow -width 220 -height 200 -scaledist 1.17 \
	...
	## Laptime
        ...
        ##new sector shit
        .lights create text 60  80 -text 0 -anchor center -fill blue  -font {Helvetica -14 bold} -tags Loop1
        .lights create text 60  100 -text 0 -anchor center -fill blue  -font {Helvetica -14 bold} -tags Loop2
        .lights create text 60  120 -text 0 -anchor center -fill blue  -font {Helvetica -14 bold} -tags Loop3
        .lights create text 60  140 -text 0 -anchor center -fill blue  -font {Helvetica -14 bold} -tags Loop4

        .lights create text 60 160 -text 0 -anchor center -fill black -font {Helvetica -14 bold} -tags Runup
