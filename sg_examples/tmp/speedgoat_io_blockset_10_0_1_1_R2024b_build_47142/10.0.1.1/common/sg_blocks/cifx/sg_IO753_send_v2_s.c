...
#define S_FUNCTION_NAME sg_IO753_send_v2_s
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    else
    {
        ...
        for (i = 0; i < mxGetM(CMD_CONFIG_ARG); i++)
        {
            if (CMD_RW(i) == 1)
            {
                ssSetInputPortWidth(S, n, (int32_T)CMD_QTY(i));
                ssSetInputPortDataType(S, n, (int32_T)CMD_DT(i));
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    if (Channel == NULL)
    {
        ...
        {
            ssSetErrorStatus(S, "Command configuration of Setup block and Send block do not match");
            return;
        }
    ...
    {
        /* Only if send/write */
        if (CMD_RW(i) == MBM_DIRECTION_WRITE)
        {
            /* Coils / discretes */
            if (CMD_DT(i) == SS_BOOLEAN)
            {
                ...
                {
                    boolean_T *Val = (boolean_T *)ssGetInputPortSignal(S, p) + n;
                    uint32_T ByteOffset = CMD_OFFSET(i) + (uint32_T)(n / 8);
                    uint32_T BitOffset = (uint32_T)(n % 8);

                    data[ByteOffset] |= ((*Val != 0) << BitOffset);
                }
            }
            /* Registers */
            else
            {
                void *port = (void *)ssGetInputPortSignal(S, p);
                memcpy(&data[CMD_OFFSET(i)], port, CMD_QTY(i) * sizeof(uint16_T));
            }
            p++;
        }
    }
    ...
    /* Copy data to DPM */
    Cifx_SndDataExt(Channel, Param->WriteLen, (void *)data);
}
