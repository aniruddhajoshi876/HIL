...
#define S_FUNCTION_NAME sg_IO751_receive_v2_s
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    PNM_PARAM_T *Param = (PNM_PARAM_T *)Channel->Param;

    if (!Param->OptAcyclic) {
        /* Process indications received from the stack */
        PNM_IndicationHandler(Channel);

        /* Process records */
        PNM_RecordHandler(Channel);

        /* Process Rx data */
        Cifx_ProcessRxData(Channel);
    }

    /* Retrieve Rx data */
    Cifx_GetRxData(Channel, DATA_LEN_ARG, (void *)ssGetOutputPortSignal(S, 0));
}
