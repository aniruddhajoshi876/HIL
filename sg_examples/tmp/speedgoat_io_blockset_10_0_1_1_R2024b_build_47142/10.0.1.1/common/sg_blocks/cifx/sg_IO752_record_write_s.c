...
#define S_FUNCTION_NAME sg_IO752_record_write_s
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    {
        if ((RECORD_ACCESSMODE(i) == 2) || (RECORD_ACCESSMODE(i) == 3))
        {
            NumPorts++;
        }
    }
    ...
    for (i = 0; i < mxGetM(RECORD_CFG); i++)
    {
        if ((RECORD_ACCESSMODE(i) == 2) || (RECORD_ACCESSMODE(i) == 3))
        {
            ssSetInputPortWidth(S, n, RECORD_BYTELEN(i));
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    if (Channel == NULL)
    {
        ...
        {
            ssSetErrorStatus(S, "RECORD configurations of Setup block and Write block do not match\n");
            return;
        }
    ...
    for (i = 0; i < mxGetM(RECORD_CFG); i++)
    {
        ...
        if ((RECORD_ACCESSMODE(i) == 2) || (RECORD_ACCESSMODE(i) == 3))
        {
            memcpy((void *)(Param->Records[i])->Data,
                   (void *)ssGetInputPortSignal(S, p),
