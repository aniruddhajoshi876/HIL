...
#define S_FUNCTION_NAME sg_IO750_send_v2_s
        ...
        else if (PORT_TYPE_ARG == 2) // one output port for each variable
        {
            ...
            for (i = 0; i < mxGetM(PDO_CFG); i++)
            {
                if (!(boolean_T)PDO_PAD(i))
                {
                    ...
                    ssSetInputPortDataType(S, n, (int32_T)PDO_DT(i));
        ...
        else // If one port for each signal
        {
            ...
            for (i = 0; i < mxGetM(PDO_CFG); i++) // Do for each PDO variable
            {
                if (!(boolean_T)PDO_PAD(i)) // Skip padding variables
                {
                    ...
                    {
                        boolean_T *p = (boolean_T *)ssGetInputPortSignal(S, n);
                        data[(uint32_T)PDO_BYTEOFFSET(i)] |= ((uint8_T)(*p > 0) << (uint8_T)PDO_BITOFFSET(i));
                    }
                    else // If non boolean
                    {
                        void *p = (void *)ssGetInputPortSignal(S, n);
                        memcpy(&data[(uint32_T)PDO_BYTEOFFSET(i)], p, (uint32_T)PDO_BITLEN(i) / 8);
                    }
                    n++;
                }
            }
