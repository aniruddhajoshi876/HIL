...
#define S_FUNCTION_NAME sg_IO751_setup_v2_s
...
// 5
#define OPT_ACYCLIC_ARG (boolean_T) mxGetPr(ssGetSFcnParam(S, 6))[0]
...
static void sg_ModelLoad(SimStruct *S)
{
    ...
    snprintf(ModuleName, sizeof(ModuleName), "IO751");
    ...
    if (0 != Cifx_AddChannel(sizeof(PNM_PARAM_T), Node, &Channel)) {
        sprintf(ErrMsg, "%s, ID %s: %s!", ModuleName, ModuleIdStr, CifxMsg);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
    ...
    snprintf(Node->FwFileName, sizeof(Node->FwFileName), "cifxpnm2.nxf");
    snprintf(Node->FwVersionStr, sizeof(Node->FwVersionStr), "3.5.0.11");
    snprintf(Node->BtldName, sizeof(Node->BtldName), "NETX100.bin");
    Node->BlockVersion = 2;
    Node->IsReInit = !sg_isFirstModelLoad();
    snprintf(Node->FwName, sizeof(Node->FwName), "PROFINET IO Controller");
    ...
    snprintf(Channel->StationTypeStr, sizeof(Channel->StationTypeStr), "Controller");
    ...
    Param = (PNM_PARAM_T *)Channel->Param;
    Param->OptAcyclic = OPT_ACYCLIC_ARG;
...
static void sg_ModelStep(SimStruct *S) { Cifx_StartBackground(); }
