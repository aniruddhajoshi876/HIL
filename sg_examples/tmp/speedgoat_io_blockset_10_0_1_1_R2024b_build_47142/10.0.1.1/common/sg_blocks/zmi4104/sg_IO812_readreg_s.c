...
#define 	S_FUNCTION_NAME 	sg_IO812_readreg_s
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    {
        if ((uint32_T)mxGetPr(PCIACCESS_ARG)[0] == 1) // 16-bit read
        ...
        else if ((uint32_T)mxGetPr(PCIACCESS_ARG)[0] == 2) // 32-bit read
		{
	        dbv_printf("readreg: before zmiRead32 0x%x\n", ptOutputPort_Read[0]);
	        ...
	        dbv_printf("readreg: after zmiRead32 0x%x\n", ptOutputPort_Read[0]);
		}
