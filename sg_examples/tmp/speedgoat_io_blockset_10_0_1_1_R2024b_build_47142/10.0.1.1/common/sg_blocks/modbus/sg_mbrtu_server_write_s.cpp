...
#define S_FUNCTION_NAME sg_mbrtu_server_write_s
...
static void sg_ModelStart(SimStruct *S)
{
    ...
    ret = Root.addInterface(SERVER_ID_ARG, false, (cInterface **)&Interface);
    if (ret != 0) {
    ...
    if (ret != 0) {
        sprintf(ErrMsg, "MBRTU | Failed to add server write block");
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    if (!Block->InitDone) {
        ...
        if (ret != 0) {
            sprintf(ErrMsg, "MBRTU | Failed to initialize server write block");
            ssSetErrorStatus(S, ErrMsg);
            return;
        }
    } else {
