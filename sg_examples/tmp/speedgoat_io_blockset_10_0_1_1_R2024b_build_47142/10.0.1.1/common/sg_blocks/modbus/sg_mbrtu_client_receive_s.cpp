...
#define S_FUNCTION_NAME sg_mbrtu_client_receive_s
...
#define UPDATE_CHANGE_ARG  (bool)mxGetPr(ssGetSFcnParam(S, 7))[0]
...
static void sg_ModelStart(SimStruct *S)
{
    ...
    ret = Transceiver->init(FUNC_CODE_ARG, REMOTE_NODE_ID_ARG, ENABLE_SEND_ARG, POLL_INTERVALL_ARG,
                            INDEX_ARG, QUANTITY_ARG, UPDATE_CHANGE_ARG, BYTE_SWAP_ARG);
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    if (Transceiver->EnaSend) {
        ...
        bool *InPort = (bool *)ssGetInputPortSignal(S, 0); /* Check if we want to send */
        if (*InPort > 0)
        {
    ...
    /* Transfer data from the protocol stack to the output port */
    Transceiver->transceive((void*)ssGetOutputPortSignal(S, 0));
}
