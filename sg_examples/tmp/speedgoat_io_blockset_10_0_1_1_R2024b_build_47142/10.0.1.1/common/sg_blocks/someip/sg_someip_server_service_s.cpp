...
#define S_FUNCTION_NAME sg_someip_server_service_s
...
static void sg_ModelStart(SimStruct *S)
{
    ...
    cServerServiceApi *Service = NULL;

    /* Add a station */
    ret = SgSomeIp->addStation(STATION_ID_ARG, SOMIEIP_STATION_TYPE_SERVER, (void **)&Station);
    if (ret != 0) {
        sprintf(ErrMsg, "SOME IP | Failed to add server %i", STATION_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }

    /* Add a service to the station */
    ret = Station->addService((cServerServiceApi **)&Service);
    if (ret != 0) {
        sprintf(ErrMsg, "SOME IP | Failed to add service to server %i", STATION_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }

    /* Initialize the service */
    ret = Service->init(SERVICE_ID_ARG, INSTANCE_ID_ARG, METHOD_ID_ARG, EVENT_ID_ARG,
                            EVENT_GROUP_ID_ARG, OUTPORT_CONNECTED, OUTPORT_DATA_LENGTH,
                            INPORT_CONNECTED, INPORT_DATA_LENGTH);
    if (ret != 0) {
        sprintf(ErrMsg, "SOME IP | Failed to initialize service of server %i\n",
                STATION_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }

    ssSetPWorkValue(S, 0, Service);
}
