...
#define S_FUNCTION_NAME sg_iec_server_sv_subscriber_s
...
static void sg_ModelStart(SimStruct *S)
{
    ...
    /* Add a node */
    ret = SVRoot.addNode(SVRoot.NodeIDs, false, (cNodeSV **)&Node);
    if (ret != 0) {
        sprintf(ErrMsg, "SV | Failed to add subscriber node %i\n", SERVER_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
    ...
    if (mxGetString(ETHERNET_PORT_ARG, EthInterfaceLabel, 1 + mxGetN(ETHERNET_PORT_ARG)) != 0) {
        ssSetErrorStatus(S, "SV | Failed to read EthInterfaceLabel parameter");
        return;
    }
    ...
    if (EthInterfaceName.empty()) {
        sprintf(ErrMsg, "SV | Ethernet interface label %s is invalid\n", EthInterfaceLabel);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
    ...
    ret = Node->init(ssGetOutputPortDataType(S, 0), 0, ssGetOutputPortWidth(S, 0), MAC_ADDRESS_ARG,
                     EthInterfaceName.c_str(), EthInterfaceLabel, APP_ID_ARG);
    if (ret != 0) {
        sprintf(ErrMsg, "SV | Failed to initialize subscriber node %i\n", SERVER_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
...
static void sg_ModelStep(SimStruct *S) // @suppress("Unused static function")
{
    ...
    cSVSNode *Node = (cSVSNode *)ssGetPWorkValue(S, 0);
