...
#define S_FUNCTION_NAME sg_iec_server_goose_publisher_s
...
#define BASE_INTERVALL_ARG (uint32_T) mxGetPr(ssGetSFcnParam(S, 7))[0]
#define GO_CB_REF_ARG      ssGetSFcnParam(S, 8)
#define DATA_SET_REF_ARG   ssGetSFcnParam(S, 9)
#define GO_ID_ARG          ssGetSFcnParam(S, 10)
#define SAMPLE_TIME_ARG    mxGetPr(ssGetSFcnParam(S, 11))[0]
...
static void sg_ModelStart(SimStruct *S)
{
    ...
    int32_t ret = 0;
    cGoosePNode *Node = NULL;

    /* Add a client node */
    ret = GooseRoot.addNode(GooseRoot.NodeIDs, true, (cNodeGoose **)&Node);
    if (ret != 0) {
        sprintf(ErrMsg, "GOOSE | Failed to add publisher node %i\n", SERVER_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
    ...
    /* Get DataSetRef Parameter */
    char_T *DataSetRef = NULL;
    DataSetRef = (char_T *)malloc(mxGetN(DATA_SET_REF_ARG) + 1);
    if (DataSetRef == NULL) {
        ssSetErrorStatus(S, "GOOSE | Not enough memory");
        return;
    }
    if (mxGetString(DATA_SET_REF_ARG, DataSetRef, 1 + mxGetN(DATA_SET_REF_ARG)) != 0) {
        ssSetErrorStatus(S, "GOOSE | Failed to read DataSetRef parameter");
        return;
    }
    stReadVect.push_back(DataSetRef);
    free(DataSetRef);

    /* Get GoID Parameter */
    char_T *GoID = NULL;
    GoID = (char_T *)malloc(mxGetN(GO_ID_ARG) + 1);
    if (GoID == NULL) {
        ssSetErrorStatus(S, "GOOSE | Not enough memory");
        return;
    }
    if (mxGetString(GO_ID_ARG, GoID, 1 + mxGetN(GO_ID_ARG)) != 0) {
        ssSetErrorStatus(S, "GOOSE | Failed to read GoID parameter");
        return;
    }
    stReadVect.push_back(GoID);
    free(GoID);

    /* Initialize the node */
    ret = Node->init(ssGetInputPortDataType(S, 0), 0, ssGetInputPortWidth(S, 0), MAC_ADDRESS_ARG,
                     EthInterfaceName, std::string(EthInterfaceLabel), stReadVect, APP_ID_ARG,
                     VLAN_ID_ARG, VLAN_PRIORITY_ARG, BASE_INTERVALL_ARG);
    if (ret != 0) {
        sprintf(ErrMsg, "GOOSE | Failed to initialize publisher node %i\n", SERVER_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
...
static void sg_ModelStep(SimStruct *S) // @suppress("Unused static function")
{
    ...
    cGoosePNode *Node = (cGoosePNode *)ssGetPWorkValue(S, 0);
