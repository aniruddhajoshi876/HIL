/*
*   File       : sg_IO614_setup_s.c
*   Description: This file includes all functions of the Simulik S-Function for the IO614 setup module.
*   Version    : $Revison$
*   Author     : $Author$
*   Date       : $Date$
...
#define S_FUNCTION_NAME     sg_IO614_setup_s
...
#define QTY_PARAM 33        // Quantity of the S-Function block parameters which are used in this S-Function code file.
...
// Module Setup
#define MOD_ID (uint32_T)mxGetPr(ssGetSFcnParam(S, 0))[0]       // Module ID of the IO614 module.
#define pDR_LTR ssGetSFcnParam(S, 1)                            // String for the driver letter on the target system.
#define pPCI_SLOT ssGetSFcnParam(S, 2)                          // PCI slot number of the IO614 module.
#define PT_1 (uint8_T)mxGetPr(ssGetSFcnParam(S, 3))[0]          // Configure port 1 as: 1 = Disabled, 2 = CAN (HS/LS)
#define PT_2 (uint8_T)mxGetPr(ssGetSFcnParam(S, 4))[0]          // Configure port 2 as: 1 = Disabled, 2 = CAN (HS/LS)
#define PT_3 (uint8_T)mxGetPr(ssGetSFcnParam(S, 5))[0]          // Configure port 1 as: 1 = Disabled, 2 = CAN (HS/LS)
#define PT_4 (uint8_T)mxGetPr(ssGetSFcnParam(S, 6))[0]          // Configure port 2 as: 1 = Disabled, 2 = CAN (HS/LS)
...
// LIN port 1
#define LIN_OP_MODE_1 (uint8_T)mxGetPr(ssGetSFcnParam(S, 25))[0]        // Operation mode for the LIN port 1 (master or slave)
#define LIN_BDR_1 (uint32_T)mxGetPr(ssGetSFcnParam(S, 26))[0]           // Baudrate of the LIN port 1.

// Hidden
#define pPROTL ssGetSFcnParam(S, 27)                                // Name of the protocol.
#define QTY_MOD (uint32_T)mxGetPr(ssGetSFcnParam(S, 28))[0]         // Quantitay of available IO613 SETUP blocks in the Simulink model.
#define pCH_NO ssGetSFcnParam(S, 29)                                // Array for the channle number of the port.
#define pPT_TYPES ssGetSFcnParam(S, 30)                             // Array for the port types of the module
#define pPROTL_HELP_IDX ssGetSFcnParam(S, 31)                       // Array of the portocol index that's the index of the port within the protocol group (e.g.: CAN 1 = 0, CAN 2 = 1, LIN 1 = 0, LIN 2 = 1)
#define IS_FD_MOD (bool)(mxGetPr(ssGetSFcnParam(S, 32))[0] ?true:false)      // Status flag if current module a CAN-FD module (=true) or not (=false)

#pragma endregion <-- Mask Parameters -->

#pragma region Globals - Static Values

const int sg_module_type = SG_MODULE_TYPE_IO614;
    ...
    if (isInit)
    {
        ...
        if (PT_1 > 1)
        {
            ...
            if (!IO61X_InitializeCanController(modIdx, ptIdx, 0, (uint8_T)mxGetPr(pPROTL_HELP_IDX)[ptIdx],
                CAN_OP_MODE_1, CAN_BDR_BRP_1, CAN_BDR_SJW_1, CAN_BDR_TSEG1_1, CAN_BDR_TSEG2_1, 0, 0, 0,
                                               0, 0, 0, CAN_STD_ACC_MASK_1, CAN_STD_ACC_CODE_1, CAN_EXT_ACC_MASK_1, CAN_EXT_ACC_CODE_1, ErrMsg))
            {
                CleanupOnError(sg_module_type, MOD_ID);
                ssSetErrorStatus(S, ErrMsg);
                return;
        ...
        // Initialize controller CAN port 2 and/or LIN port 1
        if ((2 == PT_2) || (4 == PT_2))
        {
            uint8_t ptIdx = 1;

            if (!IO61X_InitializeCanController(modIdx, ptIdx,  0, (uint8_T)mxGetPr(pPROTL_HELP_IDX)[ptIdx],
                CAN_OP_MODE_2, CAN_BDR_BRP_2, CAN_BDR_SJW_2, CAN_BDR_TSEG1_2, CAN_BDR_TSEG2_2, 0, 0, 0,
                                               0, 0, 0, CAN_STD_ACC_MASK_2, CAN_STD_ACC_CODE_2, CAN_EXT_ACC_MASK_2, CAN_EXT_ACC_CODE_2, ErrMsg))
            {
                CleanupOnError(sg_module_type, MOD_ID);
                ssSetErrorStatus(S, ErrMsg);
                return;
        ...
        if ((3 == PT_2) || (4 == PT_2))
        {
            ...
            if (!IO61X_InitializeLincotroller(modIdx, ptIdx, LIN_OP_MODE_1, LIN_BDR_1, (uint8_T)mxGetPr(pCH_NO)[ptIdx - 3], (uint8_T)mxGetPr(pPROTL_HELP_IDX)[ptIdx],
                (uint8_T)mxGetPr(pPT_TYPES)[ptIdx], ErrMsg))
            {
                CleanupOnError(sg_module_type, MOD_ID);
                ssSetErrorStatus(S, ErrMsg);
                return;
        ...
        if (PT_3 > 1)
        {
            ...
            if (!IO61X_InitializeCanController(modIdx, ptIdx, 0, (uint8_T)mxGetPr(pPROTL_HELP_IDX)[ptIdx],
                CAN_OP_MODE_3, CAN_BDR_BRP_3, CAN_BDR_SJW_3, CAN_BDR_TSEG1_3, CAN_BDR_TSEG2_3, 0, 0, 0,
                                               0, 0, 0, CAN_STD_ACC_MASK_3, CAN_STD_ACC_CODE_3, CAN_EXT_ACC_MASK_3, CAN_EXT_ACC_CODE_3, ErrMsg))
            {
                CleanupOnError(sg_module_type, MOD_ID);
                ssSetErrorStatus(S, ErrMsg);
                return;
        ...
        if (PT_4 > 1)
        {
            ...
            if (!IO61X_InitializeCanController(modIdx, ptIdx, 0, (uint8_T)mxGetPr(pPROTL_HELP_IDX)[ptIdx],
                CAN_OP_MODE_4, CAN_BDR_BRP_4, CAN_BDR_SJW_4, CAN_BDR_TSEG1_4, CAN_BDR_TSEG2_4, 0, 0, 0,
                                               0, 0, 0, CAN_STD_ACC_MASK_4, CAN_STD_ACC_CODE_4, CAN_EXT_ACC_MASK_4, CAN_EXT_ACC_CODE_4, ErrMsg))
            {
                CleanupOnError(sg_module_type, MOD_ID);
                ssSetErrorStatus(S, ErrMsg);
                return;
        ...
        if ((3 == PT_2) || (4 == PT_2))
        {
            if (!IO61X_StartLinController(modIdx, 4, ErrMsg))
            {
                CleanupOnError(sg_module_type, MOD_ID);
                ssSetErrorStatus(S, ErrMsg);
                return;
        ...
        if ((PT_1 > 1) || ((PT_2 != 1) && (PT_2 != 3)) || (PT_3 > 1) || (PT_4 > 1))
        {
...
static void sg_ModelStop(SimStruct* S)
{
	...
	
    {
        ...
        if ((3 == PT_2) || (4 == PT_2))
        {
            if (!IO61X_StopLinController(modIdx, 4))
            {
                CleanupOnError(sg_module_type, MOD_ID);
