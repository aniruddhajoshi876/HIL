...
OPERATION_MODE_SpiMacroPlayer    = 10    -- A debug mode to send commands to a SPI flash.
...
FLASHER_INTERFACE_VERSION        = 0x00030000


SPIFLASH_NAME_SIZE         = 21
SPIFLASH_ID_SIZE           = 7
OFFS_FLASH_ATTR            = 16
                           + 0
OFFS_FLASH_ATTR_acName     = 0
OFFS_FLASH_ATTR_ulSize     = 24
OFFS_FLASH_ATTR_ucIdLength = 63
OFFS_FLASH_ATTR_aucIdSend  = 64
OFFS_FLASH_ATTR_aucIdMask  = 71
OFFS_FLASH_ATTR_aucIdMagic = 78
...
function get_flasher_binary_attributes(strData)
	...
	aAttr.ulExecAddress = get_dword(strData, 36 + 1)
	aAttr.ulParameter   = get_dword(strData, 40 + 1)
	aAttr.ulDeviceDesc  = get_dword(strData, 44 + 1)
	aAttr.ulBufferAdr   = get_dword(strData, 48 + 1)
	aAttr.ulBufferEnd   = get_dword(strData, 52 + 1)
