#ifndef SG_INTERRUPT_CONFIG_LINUX_H_
#define SG_INTERRUPT_CONFIG_LINUX_H_
...
int SG_INT_MAP_FUNC_NAME_P(void** contextOut, uint32_t *data)
{
    ...
    *contextOut = (void*)context;
    ...
    if(!context->is_polling)
    {
        ...
        SG_PRINTF(DEBUG, "INT MAP -> IRQ CPU assigned: %d\n", context->irq_cpu_assigned);
        // Now we can actually migrate the IRQ Thread, because we're already in the correct thread.
        if((context->priority > 90) && (context->isModelTrigger==0)) // Base rate priority is 90, so we only start pinning if above that. Base rate is always on CPU 1.
        {
            ...
            SG_PRINTF(DEBUG, "INT MAP -> Pinning interrupt user space thread to CPU %d\n", context->irq_cpu_assigned);
    ...
    if(context->is_polling && !context->isModelTrigger)
    {
        context->irq_cpu_assigned = sg_pickNextAsyncCpu();
        SG_PRINTF(DEBUG, "INT MAP -> Asynchronous polling mode - picked CPU %d for polling thread\n", context->irq_cpu_assigned);
        if(context->irq_cpu_assigned < 0)
        {
            SG_PRINTF(ERROR, "INT MAP -> Asynchronous polling mode not supported on dual-core systems.\n");
            return -ENOSYS;
        }
        if(sg_pinThreadExclusiveCpu(context->irq_cpu_assigned) != 0)
        {
            SG_PRINTF(ERROR, "INT MAP -> Failed to pin async polling thread to CPU %d.\n",
                    context->irq_cpu_assigned);
            SG_PRINTF(ERROR, "INT MAP -> Asynchronous polling mode not recommended.\n");
            return -ENOSYS;
        }
    }
...
#endif // SG_INTERRUPT_CONFIG_LINUX_H_
