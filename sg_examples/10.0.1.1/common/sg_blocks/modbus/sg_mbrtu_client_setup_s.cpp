...
#define S_FUNCTION_NAME sg_mbrtu_client_setup_s
...
#define RESPONSE_TIMEOUT_ARG (uint32_t)mxGetPr(ssGetSFcnParam(S, 4))[0]
#define SWAP_ARG           (bool)mxGetPr(ssGetSFcnParam(S, 5))[0]
#define SAMPLE_TIME_ARG    mxGetPr(ssGetSFcnParam(S, 6))[0]
#define SUCCESSIVE_ARG     (bool)mxGetPr(ssGetSFcnParam(S, 7))[0]
...
static void sg_ModelLoad(SimStruct* S)
{
    ...
    if (ret != 0) {
        sprintf(ErrMsg, "MBRTU | Failed to add interface %i", INTERFACE_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }

    /* Initialize the interface */
    ret = Interface->init(MODULE_TYPE_ARG, MODULE_ID_ARG, CHANNEL_ARG, RESPONSE_TIMEOUT_ARG, SUCCESSIVE_ARG,
        INTERFACE_ID_ARG, SerialInfo);
    if (ret != 0) {
        sprintf(ErrMsg, "MBRTU | Failed to initialize interface %i", INTERFACE_ID_ARG);
        ssSetErrorStatus(S, ErrMsg);
        return;
    }
}
