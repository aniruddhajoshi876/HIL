...
#define 	S_FUNCTION_NAME 	sg_IO812_readreg_v2_s
...
static void mdlStart(SimStruct *S)
{
    ...
    bar2 = (volatile uint32_T*) ((uintptr_t)bar2ptr);    // BAR2 PCI MEM Space    & 0xFFFFFFF0


    
    ssSetIWorkValue(S, REMOTE_ADDR_I_IND, bar2);
//     [THIS CANNOT BE USED ANYMORE]sg_showPCIDevice(&pciinfo);   
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if ( ptInputPort_Enable[0] >= 1 )
    {
        if (ptInputPort_PciAccess[0] == 0) // 16-bit write
            ptOutputPort_Read[0] = zmiRead16(ptZmiRegs, ptInputPort_Offset[0]+ ALTHEA_REG_OFFSET);  // Add register offset so that input adress is the same as for the old module 
        
        else if (ptInputPort_PciAccess[0] == 1) // 32-bit write
		{
	    dbv_printf("readregv2: before zmiRead32 0x%x\n",ptInputPort_Offset[0]);
        ptOutputPort_Read[0] = zmiRead32(ptZmiRegs, ptInputPort_Offset[0]+ ALTHEA_REG_OFFSET); // Add register offset so that input adress is the same as for the old module
	    dbv_printf("readregv2: after zmiRead32 0x%x\n", ptInputPort_Offset[0]);
