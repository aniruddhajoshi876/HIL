...
uint8_T VmeBusStatus (SimStruct *S, BAR1TYPE * nodeio_bar)
{
    ...
    if ((tempByte & 0x80) == 0x80)
    {
		...
		//printf("Fiber optic interface error \n");
        return 1;
    }
    ...
    else if ((tempByte & 0x01) == 0x01)
    {
		...
		//printf("Remote bus power off or cable not connected \n");
        return 4;
    }
    return 0;
}
...
int VmeZmiAxisInit_intTriggerModeForceExtSample(ZMI4104TYPE* ptZmiRegs, int axis)
{
	...
	Data |= 1<< 4; //DACx16 Bit setzen um analoge SSI-Ausgabe zu verstaerken (Interferometerjustage) ~OtS 18.08.2010
