...
#define ENA_SET_STAT_PT (boolean_T) mxGetPr(ssGetSFcnParam(S, 3))[0]
#define ENA_GET_STAT_PT (boolean_T) mxGetPr(ssGetSFcnParam(S, 2))[0]
#define TIMEOUT (uint32_T) mxGetPr(ssGetSFcnParam(S, 4))[0]
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    if (ENA_SET_STAT_PT)
    {
        Cifx_SetNWStat(Channel, (uint32_T *)ssGetInputPortSignal(S, 0));
    }

    if (ENA_GET_STAT_PT)
    {
        Cifx_GetNWStat(Channel, (uint32_T *)ssGetOutputPortSignal(S, 0));
    }
}
