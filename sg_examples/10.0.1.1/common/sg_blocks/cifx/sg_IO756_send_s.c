...
#define S_FUNCTION_NAME sg_IO756_send_s
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (BLOCK_VERSION_ARG == 1) {
        ...
        if (PORT_TYPE_ARG == 1) {
            ...
            if (!ssSetNumInputPorts(S, DATA_LEN_ARG))
                return;
            ...
            ssSetInputPortWidth(S, 0, DATA_LEN_ARG);
        ...
        if (CONFIG_MODE_ARG == 0) {
            ...
            /* Get number of input assemblies */
            uint32_T n = 0;
            /* For each assembly */
            for (i = 0; i < NUM_ASSEMBLIES; i++) {
                if (ASSEMBLY_TYPE_ARG(i) == 1) {
                    /* Is input assembly */
            ...
            for (i = 0; i < NUM_ASSEMBLIES; i++) {
                if (ASSEMBLY_TYPE_ARG(i) == 1) {
                    ...
                    if ((ASSEMBLY_LEN_ARG(i) > 0)) {
                        /* Has length */
                        ssSetInputPortWidth(S, n, ASSEMBLY_LEN_ARG(i));
    ...
    if (BLOCK_VERSION_ARG == 1) {
        ...
        if (PORT_TYPE_ARG == 1) {
            ...
            for (uint32_T i = 0; i < DATA_LEN_ARG; i++) {
                uint8_t *p = (uint8_T *)ssGetInputPortSignal(S, i);
                data[i] = p[0];
            }
            Cifx_SndDataExt(Channel, DATA_LEN_ARG, (void *)data);

        } else {
            Cifx_SndDataExt(Channel, DATA_LEN_ARG, (void *)ssGetInputPortSignal(S, 0));
        }
        ...
        if (CONFIG_MODE_ARG == EIS_CONFIG_MODE_SIMPLE) {
            ...
            /* Add up overall data length over all input assemblies */
            for (uint32_T i = 0; i < NUM_ASSEMBLIES; i++) {
                if (ASSEMBLY_TYPE_ARG(i) == EIS_ASSEMBLY_TYPE_INPUT) {
                    /* Is input assembly */
            ...
            for (uint32_T i = 0; i < NUM_ASSEMBLIES; i++) {
                ...
                if (ASSEMBLY_TYPE_ARG(i) == EIS_ASSEMBLY_TYPE_INPUT) {
                    ...
                    if ((ASSEMBLY_LEN_ARG(i) > 0)) {
                        /* Has length */
                        uint8_t *p = (uint8_T *)ssGetInputPortSignal(S, n);
                        memcpy(&data[ASSEMBLY_OFFSET_ARG(i)], p, ASSEMBLY_LEN_ARG(i));
                        n++;
                    }
                }
            }
            ...
            /* Write to I/O module */
            Cifx_SndDataExt(Channel, DataLen, (void *)data);
        }
    }
}
