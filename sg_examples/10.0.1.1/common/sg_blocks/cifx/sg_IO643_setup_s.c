...
#define S_FUNCTION_NAME sg_IO643_setup_s
...
static void sg_ModelLoad(SimStruct *S)
{
    ...
    snprintf(ModuleName, sizeof(ModuleName), "IO643");
    ...
    if (0 != Cifx_AddChannel(sizeof(COM_PARAM_T), Node, &Channel))
    {
        sprintf(ErrMsg, "%s, ID %s: %s!", ModuleName, ModuleIdStr, CifxMsg);
        ssSetErrorStatus(S, ErrMsg);
        return;
    ...
    snprintf(Node->FwFileName, sizeof(Node->FwFileName), "cifxcom.nxf");
    snprintf(Node->FwVersionStr, sizeof(Node->FwVersionStr), "2.15.0.0");
    snprintf(Node->BtldName, sizeof(Node->BtldName), "NETX100.bin");
    Node->BlockVersion = 1;
    Node->IsReInit = !sg_isFirstModelLoad();
    snprintf(Node->FwName, sizeof(Node->FwName), "CANopen Master");
    snprintf(Node->ModuleIdStr, sizeof(Node->ModuleIdStr), ModuleIdStr);

    /* Set channel parameters */
    Channel->BusStartupMode = BUS_STARTUP_AUTO;
    snprintf(Channel->ProtocolStr, sizeof(Channel->ProtocolStr), "CANopen");
    snprintf(Channel->StationTypeStr, sizeof(Channel->StationTypeStr), "Master");
