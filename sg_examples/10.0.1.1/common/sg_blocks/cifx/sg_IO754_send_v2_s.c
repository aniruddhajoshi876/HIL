...
#define S_FUNCTION_NAME sg_IO754_send_v2_s
...
#define PORT_TYPE_ARG (uint8_T) mxGetPr(ssGetSFcnParam(S, 2))[0]
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    {
        uint8_T data[5760] = {0};
        uint8_T *pInPort = NULL;
        uint32_T i;

        for (i = 0; i < DATA_LEN_ARG; i++)
        {
            pInPort = (uint8_T *)ssGetInputPortSignal(S, i);
            data[i] = pInPort[0];
        }
