...
#define S_FUNCTION_NAME sg_IO750_sdo_write_s
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    {
        if ((SDO_ACCESSMODE(i) == 2) || (SDO_ACCESSMODE(i) == 3))
        {
    ...
    if (!ssSetNumInputPorts(S, NumPorts))
        return;
    ...
    for (i = 0; i < mxGetM(SDO_CFG); i++)
    {
        if ((SDO_ACCESSMODE(i) == 2) || (SDO_ACCESSMODE(i) == 3))
        {
            if (SDO_SUBINDEX(i) != 0 || (SDO_NUMELEMENTS(i) == 0))
            {
                ...
                ssSetInputPortDataType(S, n, (int32_T)SDO_DT(i));
...
static void sg_ModelStep(SimStruct *S)
{
    ...
    for (i = 0; i < Param->NumSdo; i++)
    {
        ...
        {
            /* Read direction */
            if ((Sdo->AccessMode == 2) || (Sdo->AccessMode == 3))
            {
                if (p < (uint32_T)ssGetNumInputPorts(S))
                {
                    void *Port = (void *)ssGetInputPortSignal(S, p);

                    memcpy((void *)Sdo->WriteValue, Port, Sdo->ByteLen);
