...
#define API_REV    644               /* Release version number */
#define API_VER    "6.44"            /* Release version for the resource file */
#define API_RC_VER 6,4,4,0           /* Release version for the resource file */
#define API_TYPE "Release 10/17/2011"  /* Release type is BETA or RELEASE   */
#define API_MISC "1553A/1553B by RT address, extended status"
...
#define API_FIRMWARE "5.18/5.10/5.02/5.00/4.66/4.64/4.50/4.49/4.41/4.40/3.93/3.88"
#define API_OS_SUPPORT "64-Bit Windows 32-Bit Windows, Linux, Solaris, VxWorks, LynxOS, QNX, Integrity"
...
typedef BT_INT CEI_THREAD;
typedef BT_INT CEI_EVENT;
typedef BT_INT CEI_MUTEX;
/*---------------------------------------------------------------------------*
 *                Compiler and Operating System Specific Constants
 *---------------------------------------------------------------------------*/
 
#include "1553_defines.h"
...
#define RAR15XMC    0x300    /* This is the RAR15XMC multi-protocol XMC   */
...
#define BT1553_INT_BIT_COUNT       0x00004000L // bit count error       ** (Not used by F/W 5.00 or later)
...
************************************************/
#define EI_ENH_ZEROXNG       16     // currently not implemented in the API. RTVAL Tester only
...
char *BusToolsError(int code);

// xPC Target specific functions
BT_INT xpcQPCIInit( uintptr_t base_addr, BT_INT channel );
BT_INT xpcQPCIClose( BT_INT cardnum );
...
NOMANGLE BT_INT CCONV API_Init(
   ...
   uintptr_t,     // (i) base address of board memory area
...
NOMANGLE BT_INT CCONV BusTools_API_InitExtended(
   ...
   uintptr_t base_address,  // (i) base address of board/carrier memory area
...
NOMANGLE BT_INT CCONV BusTools_GetTermEnable(
   BT_UINT cardnum,    // (i) card number
   BT_U16BIT *tEnable); // (o) Enable setting
...
NOMANGLE BT_INT CCONV BusTools_EI_EnhEbufWrite(
   BT_UINT cardnum,         // (i) card number (0 - based)
   BT_UINT errorid,         // (i) number of error injection buffer to write
   BT_UINT enhData,         // (i) enhanced data 
   API_EIBUF * ebuf);        // (i) pointer to buffer values to be written to HW
...
#define API_INSTALL_ERROR        0x7FFFFFFF   // CEI-ERROR -- Install Error check -> FW:0x80000000 is not supported by aarch64
/**********************************************************************
...
NOMANGLE BT_INT CCONV BusTools_DMA_Setup2(BT_UINT cardnum, 
                                          BT_UINT dma_channel, 
                                          BT_VOID* host_addr,
...
NOMANGLE BT_INT CCONV BusTools_DMA_Transfer(BT_UINT cardnum,
                                            BT_UINT dma_channel);

NOMANGLE BT_INT CCONV BusTools_DMA_Init(BT_UINT cardnum,
                                        BT_UINT dma_channel,
                                        BT_U32BIT size,
                                        BT_U32BIT flag);

NOMANGLE BT_INT CCONV BusTools_DMA_Close(BT_UINT cardnum,
                                         BT_UINT dma_channel);

NOMANGLE BT_INT CCONV BusTools_DMA_Read(BT_UINT cardnum,
                                        BT_UINT dma_channel,
                                        BT_UINT board_addr,
                                        BT_UINT byte_count,
                                        BT_UINT* buff);
