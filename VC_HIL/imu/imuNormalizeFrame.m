function frame = imuNormalizeFrame(source)
%IMUNORMALIZEFRAME Validate one raw IMU frame into the canonical frame shape.
...
%   sensor), so it now lives with the rest of the IMU driver code.
%   SENSORNORMALIZER.SENSORFRAMEORDER takes already-normalized frames from
%   this and LWSNORMALIZEFRAME and orders/filters them generically.
frame = emptyFrame();
frame.kind = 'imu';
