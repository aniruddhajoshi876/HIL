...
if nargin < 1 || isempty(dictionaryPath)
    % This file lives in VC_HIL/inverter/build (version-controlled source,
    % not a generated artifact -- see the header note; grouped alongside
    % INVERTER_HIL.MLDATX, the one generated artifact that stayed under
    % inverter/ rather than moving to VC_HIL/build). The dictionary it
    % writes into is a DIFFERENT, sibling build folder -- VC_HIL/build,
    % where BUILD_INVERTER_HIL_MODEL writes inverter_hil.slx/.sldd -- three
    % levels up from this file, not two.
    dictionaryPath = fullfile( ...
        fileparts(fileparts(fileparts(mfilename('fullpath')))), ...
        'build', 'inverter_hil.sldd');
end
...
applied = pedalCalibrationConstants();
