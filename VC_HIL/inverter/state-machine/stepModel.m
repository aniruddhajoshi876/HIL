...
for index = 1:numel(frames)
    bank = receiveControlFrame(bank, frames(index), tickMs);
end
...
[nextState, stateOutput] = stepSystemState(state, sysInput, config);

plantInput = defaultPlantInput(cal);
for channel = 1:4
...
[nextPlantState, plantOutput] = stepPlant(plantState, plantInput, cal);
