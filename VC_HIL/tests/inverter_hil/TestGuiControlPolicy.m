classdef TestGuiControlPolicy < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function canDrivingLocksOnlyPedalsAndDefaultsFalse(testCase)
            % canDriving is write-ownership arbitration between the GUI and
            % CarMaker's CAN pedal demand, NOT part of the INTERLOCKS
            % REMOVED 2026-08-02 decision -- unlike every flag in that set,
            % this one DOES gate a group when true. It must gate pedals
            % only, and every existing 3-arg caller (the whole rest of this
            % file) must see identical behavior to before this parameter
            % existed.
            interlocks = TestGuiControlPolicy.healthyInterlocks();
            ...
            % Every other group is unaffected by canDriving.
            ...
            for index = 1:numel(badValues)
                ...
                testCase.verifyEqual(policy.reason, 'malformed_canDriving', ...
                    sprintf('bad canDriving %d', index));
            end
        end
