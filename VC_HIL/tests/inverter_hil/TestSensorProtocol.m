classdef TestSensorProtocol < matlab.unittest.TestCase
    ...
    methods (Test)
        function SensorTxIdsMatchTheProtocolContracts(testCase)
            ...
            % The four scalar-group frames the MTi appends to every inertial
            % group, in transmit order (MT1604P 6.1.1-6.1.3, 6.2.1).
            expectedScalar = uint32([imu.groupCounter.id, imu.sampleTime.id, ...
                imu.statusWord.id, imu.errorCode.id]);
            [actualImu, imuDlc] = imuTxIds();
            [actualLws, lwsDlc] = lwsTxIds();
            [actualScalar, scalarDlc] = imuScalarTxIds();
            ...
            testCase.verifyEqual(actualScalar, expectedScalar, ...
                ['imuScalarTxIds must match imuProtocol in both value and ' ...
                'transmit order.']);
            testCase.verifyEqual(imuDlc, uint8([6 6 6]));
            testCase.verifyEqual(lwsDlc, uint8([5 2]));
            testCase.verifyEqual(scalarDlc, uint8([ ...
                imu.groupCounter.dlc, imu.sampleTime.dlc, ...
                imu.statusWord.dlc, imu.errorCode.dlc]));
        end
        ...
        function MtiScalarFramesArePackedBigEndianUnsigned(testCase)
            ...
            % PACKMTI680SCALARPAYLOAD is the codegen-friendly payload-only
            % packer the deployed model's Scalar Sensor Payloads block uses.
            % It must produce byte-for-byte the same payload as the full
            % frame packer for every scalar kind and value, or the target
            % and the host simulator diverge on the wire.
            parityCases = { ...
                'groupCounter', 4660; 'groupCounter', 0; ...
                'groupCounter', 65535; 'sampleTime', 1e6; ...
                'sampleTime', 0; 'sampleTime', 4294967295; ...
                'statusWord', 3; 'errorCode', 0; 'errorCode', 1};
            for index = 1:size(parityCases, 1)
                kind = parityCases{index, 1};
                value = parityCases{index, 2};
                testCase.verifyEqual( ...
                    packMti680ScalarPayload(kind, value), ...
                    packMti680ScalarFrame(kind, value).payload, ...
                    sprintf('scalar payload parity failed for %s=%g', ...
                    kind, value));
            end
            testCase.verifyError(@() packMti680ScalarPayload('groupCounter', ...
                70000), 'mti680:PayloadRange');
            testCase.verifyError(@() packMti680ScalarPayload('acceleration', ...
                1), 'mti680:UnsupportedPayload');
        end
