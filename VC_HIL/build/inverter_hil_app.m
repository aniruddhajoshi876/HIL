    ...
    methods (Access = private)
        ...
        function refreshLiveIo(app)
            ...
            if live.known
                ...
                if isfield(live.can, 'carMakerKnown') && live.can.carMakerKnown
                    cm = live.can.carMakerDiagnostics;
                    app.Telemetry.can.carMakerKnown = true;
                    app.Telemetry.can.carMakerDiagnostics.busLoadPercent = ...
                        cm.busLoadPercent;
                    app.Telemetry.can.carMakerDiagnostics.busOff = cm.busOff;
                    app.Telemetry.can.carMakerDiagnostics.recoveryCount = ...
                        cm.recoveryCount;
                    app.Telemetry.can.carMakerDiagnostics.transmitOverrun = ...
                        cm.transmitOverrun;
                    app.Telemetry.can.carMakerDiagnostics.receiveOverrun = ...
                        cm.receiveOverrun;
                    app.Telemetry.can.carMakerDiagnostics.errorWarning = ...
                        cm.errorWarning;
                end
                ...
                blankCan = inverterhilgui.live_telemetry.blankTelemetry().can;
                app.Telemetry.can.diagnostics = blankCan.diagnostics;
                app.Telemetry.can.carMakerKnown = blankCan.carMakerKnown;
                app.Telemetry.can.carMakerDiagnostics = ...
                    blankCan.carMakerDiagnostics;
        ...
        function text = rxFrameSignalText(app, id, channel)
            ...
            [accepted, ~, command] = decodeControlFrame( ...
        ...
        function text = canDiagnosticsText(app, snapshot)
            %CANDIAGNOSTICSTEXT Two-line CAN diagnostics strip, one per bus.
            %   The IO614 CAN bus is split: channel 2 / Port A is the real-VC
            %   bus (control RX, status + sensor TX), channel 1 / Port B is
            %   the CarMaker bus (0x500 RX, 0x501/0x502 TX). Each has its own
            %   IO614 CAN Status block.
            vcLine = app.canBusDiagnosticsLine('VC BUS', ...
                snapshot.can.diagnostics, true);
            if isfield(snapshot.can, 'carMakerDiagnostics')
                carMakerDiag = snapshot.can.carMakerDiagnostics;
            else
                carMakerDiag = struct();
            end
            carMakerLine = app.canBusDiagnosticsLine('CARMAKER BUS', ...
                carMakerDiag, false);
            text = sprintf('%s\n%s', vcLine, carMakerLine);
        end

        function line = canBusDiagnosticsLine(app, label, diagnostics, showWrites)
            %CANBUSDIAGNOSTICSLINE One bus's diagnostics as a single line.
            writeText = '--';
            ...
            line = sprintf(['%-13s WRITE %s | BUS LOAD %s | TX OVERRUN %s | ' ...
                'RX OVERRUN %s | ERROR WARNING %s | BUS-OFF %s | ' ...
                'RECOVERIES %s'], label, writeText, ...
                inverterhilgui.live_telemetry.formatMeasurement( ...
                app.diagField(diagnostics, 'busLoadPercent'), NaN, '%', ...
                false).value, ...
                inverterhilgui.live_telemetry.formatPinState( ...
                app.diagField(diagnostics, 'transmitOverrun')).text, ...
                inverterhilgui.live_telemetry.formatPinState( ...
                app.diagField(diagnostics, 'receiveOverrun')).text, ...
                inverterhilgui.live_telemetry.formatPinState( ...
                app.diagField(diagnostics, 'errorWarning')).text, ...
                inverterhilgui.live_telemetry.formatPinState( ...
                app.diagField(diagnostics, 'busOff')).text, ...
                inverterhilgui.live_telemetry.formatMeasurement( ...
                app.diagField(diagnostics, 'recoveryCount'), NaN, '', ...
                false).value);
        end

        function value = diagField(~, s, name)
            %DIAGFIELD A diagnostics field, or empty when the struct lacks it.
