...
% Nine Ephorus status frames plus the four synchronized sensor frames
% (MTi-680G acceleration 0x034, rate-of-turn 0x032 and velocity 0x076,
% Bosch LWS 0x2B0).
assert(sum(strcmp(references, 'speedgoatlib_IO614/CAN Write ')) == 13, ...
    'Expected thirteen linked IO614 CAN Write blocks.');
% Each CAN Write consumes a CAN_MESSAGE, which only a CAN Pack produces.
assert(sum(strcmp(references, 'canmsglib/CAN Pack')) == 13, ...
    'Expected thirteen linked CAN Pack blocks feeding the CAN Write blocks.');
...
assert(isequal(sort(ids), sort(hex2dec( ...
    ...
    '032', '034', '076', '2B0'}))), ...
    ['Expected the nine Ephorus status IDs plus the four sensor IDs ' ...
    '(MTi 0x032/0x034/0x076, Bosch LWS 0x2B0).']);
