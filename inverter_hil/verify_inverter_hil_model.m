...
% Nine Ephorus status frames plus the three synchronized sensor frames
% (MTi-680G acceleration 0x034 and rate-of-turn 0x032, Bosch LWS 0x2B0).
assert(sum(strcmp(references, 'speedgoatlib_IO614/CAN Write ')) == 12, ...
    'Expected twelve linked IO614 CAN Write blocks.');
% Each CAN Write consumes a CAN_MESSAGE, which only a CAN Pack produces.
assert(sum(strcmp(references, 'canmsglib/CAN Pack')) == 12, ...
    'Expected twelve linked CAN Pack blocks feeding the CAN Write blocks.');
...
assert(isequal(sort(ids), sort(hex2dec( ...
    ...
    '032', '034', '2B0'}))), ...
    ['Expected the nine Ephorus status IDs plus the three sensor IDs ' ...
    '(MTi 0x032/0x034, Bosch LWS 0x2B0).']);
