...
deleteExistingLine(path, 'Module 2 DI01-DI08/1', 'VCU Input Mux/2');
for k = 1:5
    deleteExistingLine(path, sprintf('Port A RX Selector/%d', k), ...
        sprintf('VCU Input Mux/%d', k+2));
end
add_line(path, 'Module 2 Analog Mux/1', 'VCU Input Mux/1', 'autorouting', 'on');
add_line(path, 'Module 2 DI01-DI08/1', 'VCU Input Double 2/1', 'autorouting', 'on');
